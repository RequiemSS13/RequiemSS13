				...
				if(H.stat != DEAD)
					if(H.key)
						...
						if(P)
							P.add_experience(1)
							if(H.mind)
								if("[H.mind.assigned_role]" == "Seneschal" || "[H.mind.assigned_role]" == "Sheriff" || "[H.mind.assigned_role]" == "Seneschal" || "[H.mind.assigned_role]" == "Chantry Regent" || "[H.mind.assigned_role]" == "Baron" || "[H.mind.assigned_role]" == "Dealer")
									P.add_experience(2)
							if(!HAS_TRAIT(H, TRAIT_NON_INT))
								if(won)
									if(H.vampire_faction == won)
