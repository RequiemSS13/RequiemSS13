...
/mob/living/carbon/human/npc/on_hit(obj/projectile/hit_projectile)
	. = ..()
	if(hit_projectile)
		if(hit_projectile.firer)
			Aggro(hit_projectile.firer, TRUE)
			var/witness_count
			for(var/mob/living/carbon/human/npc/witness_npc in viewers(7, usr))
				witness_npc.Aggro(hit_projectile.firer)
				...
				if(witness_count > 1)
					...
					for(var/obj/item/p25radio/police/p_radio in GLOB.p25_radios)
						if(p_radio.linked_network == "police")
							p_radio.announce_crime("victim", get_turf(src))
